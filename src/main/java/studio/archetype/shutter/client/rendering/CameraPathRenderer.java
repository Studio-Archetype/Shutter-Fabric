    ...
    public void render(MatrixStack stack, OutlineVertexConsumerProvider consume, Vec3d camera) {
        ...
        for (int i = 0; i < keyframes.size() - 1; i++) {
            ...
            for(float j = 0; j <= 1; j += PRECISION) {
                double x = HermiteMath.interpolate(new double[] {p1.getX(), p2.getX(), p3.getX(), p4.getX()}, j, 2, 1);
                double y = HermiteMath.interpolate(new double[] {p1.getY(), p2.getY(), p3.getY(), p4.getY()}, j, 1, 2);
                double z = HermiteMath.interpolate(new double[] {p1.getZ(), p2.getZ(), p3.getZ(), p4.getZ()}, j, 2, 1);

                Matrix4f model = stack.peek().getModel();
                VertexConsumer vert = consume.getBuffer(RenderLayer.getLines());

                vert.vertex(model, (float)previous.getX(), (float)previous.getY(), (float)previous.getZ()).color(0F, 0F, 0F, 0.4F).next();
                vert.vertex(model, (float)x, (float)y, (float)z).color(0F, 0F, 0F, 0.4F).next();
