...
public class NetworkHandler {
    ...
    public static void register() {
        ServerSidePacketRegistry.INSTANCE.register(PACKET_KEY_NODE_CREATE, (ctx, data) -> {
            ...
            Shutter.PATH_COMPONENT.get(ctx.getPlayer().getEntityWorld()).addNode(id, node);
        });
        ...
        ServerSidePacketRegistry.INSTANCE.register(PACKET_KEY_PATH_SHOW, (ctx, data) -> {
            ...
            Shutter.PATH_COMPONENT.get(ctx.getPlayer().getEntityWorld()).togglePathVisualization(player, id);
        });
    }
}
